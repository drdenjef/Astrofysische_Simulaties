...
int main(){
    ...
    int aantal_deeltjes = 10;
    std::vector<Vec> a = randomVec(aantal_deeltjes);
    std::vector<Vec> b = randomVec(aantal_deeltjes);
    std::vector<double> c = randommass(aantal_deeltjes);

    std::vector<double> m = massrescaling(c);
    std::vector<Vec> p = posrescaling(m, a);
    std::vector<Vec> v = velrescaling(m, b);

    double Ek = EnergieK(v, m);
    double Ep = EnergieP(p, m);

    std::cout << Ek << Ep << std::endl;
