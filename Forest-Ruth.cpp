...
vector<vector<vector<Vec>>> ForestRuth(vector<Vec> pos, vector<Vec> snel, vector<double> mass, double h, int aantal, string naam) {
	...
	for (int k = 0; k < 1; k += h) {
		for (int i = 0; i < aantal; i += 1) {
			...
			// berekenen van de versnelling
			acc = a(mass, pos, i, aantal);

			//substeps 4 & 5
			snel[i] = snel[i] + ((1 - 2 * theta) * h) * acc;
			pos[i] = pos[i] + 0.5*(1 - theta)*h*snel[i];
